...
export function ModalHeader({ title, description, className }: { title: string; description?: string; className?: string }) {
  return (
    <div className={cn('flex items-start justify-between px-6 py-5 border-b border-border-default', className)}>
...
export function ModalFooter({ children, className }: { children: ReactNode; className?: string }) {
  return <div className={cn('flex items-center justify-end gap-3 px-6 py-4 border-t border-border-default bg-surface-50/50 dark:bg-surface-950/30', className)}>{children}</div>;
}
