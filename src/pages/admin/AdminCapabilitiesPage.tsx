    ...
    <div className="p-4 rounded-xl border border-border-default bg-white dark:bg-surface-900 hover:border-brand-300 dark:hover:border-brand-700 hover:shadow-sm transition-all">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-body-sm text-content-tertiary mt-1 leading-relaxed">{cap.description}</p>
      ...
      {cap.supportedParameters.length > 0 && (
        <details className="mt-3 ml-[3.25rem]">
          <summary className="text-body-xs text-content-tertiary cursor-pointer hover:text-content-secondary transition-colors">
            Parameters ({cap.supportedParameters.length})
          </summary>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {cap.supportedParameters.map(param => (
              <span key={param.name} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-surface-100 dark:bg-surface-700 text-body-xs text-content-secondary">
                <code className="font-mono">{param.name}</code>
                <span className="text-content-tertiary">({param.type})</span>
