...
function ToastItem({ toast, onRemove }: { toast: ToastItem; onRemove: React.Dispatch<React.SetStateAction<ToastItem[]>> }) {
  const variants = {
    default: 'bg-white dark:bg-[var(--color-surface-900)] border-[var(--color-border-default)]',
  ...
  const icons = {
    default: <svg className="h-5 w-5 text-[var(--color-brand-500)]" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" /></svg>,
  ...
  return (
    <div className={`flex items-start gap-3 p-4 rounded-xl border shadow-[var(--shadow-elevation-3)] animate-in slide-in-from-bottom ${variants[toast.variant]}`} role="alert">
      <div className="flex-shrink-0 mt-0.5">{icons[toast.variant]}</div>
      <p className="flex-1 text-sm text-[var(--color-content-primary)]">{toast.message}</p>
      <button
        onClick={() => onRemove(prev => prev.filter(t => t.id !== toast.id))}
        className="flex-shrink-0 text-[var(--color-content-tertiary)] hover:text-[var(--color-content-primary)]"
        aria-label="Dismiss"
      >
